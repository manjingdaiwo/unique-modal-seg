...
class Supervision_Train(pl.LightningModule):
    """pl.LightningModule, https://blog.csdn.net/qian_chun_qiang/article/details/109078387"""
    def __init__(self, config):
        ...
        self.metrics_train = Evaluator(num_class=config.num_classes)   #训练指标类：Evaluator()
    ...
    def training_step(self, batch, batch_idx):
        ...
        pre_mask = pre_mask.argmax(dim=1)   #预测标签
    ...
    def training_epoch_end(self, outputs):
        ...
        print("train per_class iou", iou_value)
        self.metrics_train.reset()
        loss = torch.stack([x["loss"] for x in outputs]).mean()
        log_dict = {"train_loss": loss, 'train_mIoU': mIoU, 'train_F1': F1, 'train_OA': OA}
        self.log_dict(log_dict, prog_bar=False)  #prog_bar记录到进度条
    ...
    def validation_epoch_end(self, outputs):
        ...
        self.log_dict(log_dict, prog_bar=False)   #prog_bar=True记录到进度条
...
def main():
    ...
    """Modelcheckpoinz:通过监控设置定期保存模型:save_top_k:保存最好的k个模型,monitpr:被监控的值，
    save_lase:保存最后一个epoch的模型,mode:用最好的覆盖已有的，
    dirpath:模型的保存路径,filename模型名"""
    ...
    logger = CSVLogger('lightning_logs', name=config.log_name)  #日志记录器

    model = Supervision_Train(config)    #加载模型
    ...
    trainer.fit(model=model)   #模型分布式训练
