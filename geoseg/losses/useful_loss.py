...
class  UnetFormerLoss(nn.Module):
    ...
    def forward(self, logits, labels, mode='val'):
        if mode == 'train' and len(logits) == 2:
            logit_main, logit_aux = logits
            loss = self.main_loss(logit_main, labels) + 0.4 * self.aux_loss(logit_aux, labels)
        elif mode == 'train' and len(logits) == 3:
            logit_main, logit_aux, logit_aux1 = logits
            loss = self.main_loss(logit_main, labels) + 0.4 * self.aux_loss(logit_aux, labels)+ 0.4 * self.aux_loss(logit_aux1, labels)
        else:
