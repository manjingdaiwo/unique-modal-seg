...
class Mynet(nn.Module):
    ...
    def forward(self, x):
        ...
        """decoder"""
        f_high4 = self.aspp(x4) #1/32, 256
        f_high4 = F.interpolate(f_high4, x3_size[2:], mode='bilinear', align_corners=True)  #1/16, 256


        f_high2 = self.cbr2(x3)  #1/16, 256 
        f_high3 = F.interpolate(f_high4, x3_size[2:], mode='bilinear', align_corners=True)  #1/16, 256
        f_high2 = torch.cat((f_high2, f_high3), dim=1)    #1/16, 512
        f_high2 = self.conv2(f_high2)    #1/16, 256
