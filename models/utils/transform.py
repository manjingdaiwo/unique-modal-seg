...
def rand_flip2(img, label, edge):
    r = random.random()
    # showIMG(img.transpose((1, 2, 0)))
    if r < 0.25:
        return img, label, edge
    elif r < 0.5:
        return np.flip(img, axis=0).copy(), np.flip(label, axis=0).copy(), np.flip(edge, axis=0).copy()
    elif r < 0.75:
        return np.flip(img, axis=1).copy(), np.flip(label, axis=1).copy(), np.flip(edge, axis=1).copy()
    else:
        return img[::-1, ::-1, :].copy(), label[::-1, ::-1].copy(), edge[::-1, ::-1].copy()
...
def slidding_crop_WC(ims, labels,edges, crop_size_local, scale=8):
    crop_imgs = []
    crop_labels = []
    crop_edges = []
    c_h = crop_size_local
    c_w = crop_size_local
    label_dims = len(labels[0].shape)
    for img, label, edge in zip(ims, labels, edges):
        h = img.shape[0]
        w = img.shape[1]
    ...
    # print('Sliding crop finished. %d images created.' %len(crop_imgs))
    return crop_imgs, crop_labels, crop_edges
...
def random_crop(img, label, edge, size):
    # print(img.shape)
    h = img.shape[0]
    w = img.shape[1]
    c_h = size[0]
    ...
    if h < c_h or w < c_w:
        ...
        # print('%d %d %d %d'%(s_h, e_h, s_w, e_w))
        return crop_im, crop_label, crop_edge

def random_crop2(img, label, edge, crop_size_local, scale):
    # print(img.shape)
    h, w = img.shape[:2]
    ...
    if h < crop_size_local or w < crop_size_local:
        ...
        h_seed = random.randint(0, h-crop_size_local)
        w_seed = random.randint(0, w-crop_size_local)
        ...
        crop_edge = edge[start_h:end_h, start_w:end_w]
